...
int PianoAdvancedBoogieWoogie::ArrangeBoogieWoogie(Song *s) {
 ...
 int bw_seed = this->rndInt(0,INT_MAX);
 int bw_seed_chorus = this->rndInt(0,INT_MAX);
 int bw_seed_alt = this->rndInt(0,INT_MAX);
 ...
 for(int i = 0;i < s->getParts();++i ){
       if(s->getPart(i)->getArrHint() == 0) {
           s->addRenderEvent("Simple Chords",this->rndInt(0,INT_MAX),1,s->getPartStartBar(i),s->getPartEndBar(i),1,this->createTime(0,0),0.8);
       ...
       s->addRenderEvent("Simple Chords",this->rndInt(0,INT_MAX),2,s->getPartEndBar(i) - 1,s->getPartEndBar(i),0,this->createTime(0,0),0.8);
       if(i == s->getParts() - 1) {
           s->addRenderEvent("Simple Chords",this->rndInt(0,INT_MAX),2,s->getPartEndBar(i) - 1,s->getPartEndBar(i),2,this->createTime(0,0),0.8);
           s->addRenderEvent("Simple Chords Smooth",this->rndInt(0,INT_MAX),2,s->getPartEndBar(i) - 1,s->getPartEndBar(i),3,this->createTime(0,s->getUniquePart(s->getPart(i)->getUniquePart())->getMetrum() / 2),0.8);
    }
    else {
           s->addRenderEvent("Simple Chords",this->rndInt(0,INT_MAX),2,s->getPartEndBar(i) - 1,s->getPartEndBar(i),1,this->createTime(0,s->getUniquePart(s->getPart(i)->getUniquePart())->getMetrum() / 2),0.75);
    }
 }
 return 1;
}
